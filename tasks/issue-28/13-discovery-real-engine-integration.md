...
- [ ] In the same test, assert: the `(deployment, network, host)` subnet is present in `state.Subnets`; the created docker network's options carry MTU `1420`; a type-A query for `<service>` against the bridge gateway returns the container's IP; a query for `<service>.<deployment>.jaco.internal` returns the same IP.

## Acceptance criteria
- [ ] `go test -tags docker -run TestDiscovery ./internal/discovery/` passes on a host with Docker (+ `CAP_NET_BIND_SERVICE` for the responder bind).
- [ ] `go vet -tags docker ./internal/discovery/...` exits 0.
