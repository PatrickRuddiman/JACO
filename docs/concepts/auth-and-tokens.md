...
JACO has RPC credentials, an external state-encryption keyring, and a
filesystem-permission trust boundary. Knowing which one a command needs prevents the most common
...
Every member's trusted in-memory FSM holds its private signing key.
Replication, persistence and backup export protect it with state-encryption
envelopes; live-node compromise still exposes that authority. Off-node CLI calls pin this cert via
...
An independently provisioned versioned 256-bit keyring protects replicated
payloads, durable logs/snapshots, backups and certificate caches. It is not
an operator token or TLS credential and is never distributed through Raft,
join responses or backups. Every member must have the same complete ring.
See [provisioning, offline migration and recovery](../operations/state-encryption.md).
...
**At-rest posture.** Registry secrets, resolved manifests, the cluster
CA private key (`ca_key` in `ClusterMeta`) and ACME private blobs
(`CertBlob`) share an authenticated envelope-encryption boundary before
Raft persistence. Snapshots, backup content and fallback caches are
encrypted too. Wrapping keys remain outside those artifacts. Legacy
plaintext logs, free pages, caches and backups require explicit
[fresh-copy migration and retention handling](../operations/state-encryption.md);
future encrypted writes alone do not sanitize history.
...
part of the per-deployment `compose_yaml` field. Those bytes are encrypted
as application state, not replaced by secret references. Runtime consumers
still receive their original values. Routine Status/Watch views omit the
manifests; [`jaco get deployment --show-secrets`](../cli/get.md) is the
