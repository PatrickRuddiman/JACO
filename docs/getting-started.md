...
Before the service start, independently provision the **same complete
external state keyring on every host** following
[state encryption](operations/state-encryption.md#provision-one-keyring-for-the-cluster).
No per-node autogenerated key or join-time key distribution exists. For an
existing plaintext cluster, use the offline migration guide instead of this
fresh-cluster procedure.

With keys provisioned, the fresh daemon is **uninitialized** — every RPC except
