...
| `volumes`      | named volumes and host bind mounts. Default named volumes use `jaco_v2_<digest>`, derived from the cluster ID, deployment and volume key, with ownership checked before reuse. Top-level `volumes.<key>.name:` (or `external: true`) is the compose-portable escape hatch for intentional sharing. See [Migration → How JACO names volumes](../operations/migration.md#how-jaco-names-volumes) before upgrading existing storage |
