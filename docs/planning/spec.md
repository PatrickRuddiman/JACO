...
- The supported jaco.yaml fields are: `name` (deployment name), `compose` (reference to the docker compose file for this deployment), `services` (map of service name → `{replicas: int ≥ 0, hosts: [hostname, …], placement: spread | pack}`; `hosts` and `placement` are mutually exclusive; if neither is set, `placement: spread` applies), `routes` (list of `{domain, service, port, tls: auto | off}`).
...
- The only fields permitted in a jaco.yaml are: `name`, `compose`, `services`, `routes`. The only sub-fields permitted under `services.<name>` are: `replicas`, `hosts`, `placement`. The only sub-fields permitted under `routes[*]` are: `domain`, `service`, `port`, `tls`. Unknown fields cause apply to be rejected.
