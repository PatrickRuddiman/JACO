...
// docker engine's mount.Mount shape. Inline binds and named volumes
// flow straight to docker; anonymous volumes get an empty Source so
// docker picks the name itself.
func toDockerMounts(spec compose.ContainerSpec) []mount.Mount {
	in := spec.Mounts
	if len(in) == 0 {
		return nil
	}
