...
func TestJoinRequiresProvisionedCA(t *testing.T) {
	...
	withHeaders, _ := pem.Decode(trustedCA)
	withHeaders.Headers = map[string]string{"Comment": "not certificate data"}
	for name, caPEM := range map[string][]byte{
		...
		"headers":          pem.EncodeToMemory(withHeaders),
		"opaque-prefix":    bytes.Join([][]byte{[]byte("opaque data"), trustedCA}, []byte("\n")),
		"opaque-gap":       bytes.Join([][]byte{trustedCA, []byte("opaque data"), trustedCA}, []byte("\n")),
		"opaque-tail":      bytes.Join([][]byte{trustedCA, []byte("opaque data")}, []byte("\n")),
		"malformed-gap": bytes.Join([][]byte{trustedCA,
		...
		t.Run(name, func(t *testing.T) {
			...
			if _, err := os.Stat(filepath.Join(s.dataDir, "node")); !os.IsNotExist(err) {
				t.Errorf("invalid provisioned CA was persisted: %v", err)
			}
...
func TestJoinPersistsProvisionedRotationBundle(t *testing.T) {
	...
	bundle := bytes.Join([][]byte{[]byte("\t "), oldCA, newCA, []byte("\t ")}, []byte("\n"))
	bundle = bytes.ReplaceAll(bundle, []byte("\n"), []byte("\r\n"))
	serverCert := peerCertificate(t, oldCA, oldKey, "peer", net.ParseIP("127.0.0.1"))
	for name, returnedCA := range map[string][]byte{"single-ca": newCA, "ca-bundle": bundle} {
		t.Run(name, func(t *testing.T) {
			addr, peer := startRecordingPeer(t, serverCert, func(req *pb.NodeJoinRequest) (*pb.NodeJoinResponse, error) {
				...
				chain := bytes.Join([][]byte{leaf, newCA}, nil)
				return &pb.NodeJoinResponse{
					ClusterId: "rotation-test", CaCert: returnedCA,
					SignedCert: bytes.ReplaceAll(chain, []byte("\n"), []byte("\r\n")),
