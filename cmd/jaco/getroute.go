...
// getCmd is the parent for read-only `jaco get <resource>` lookups. Today it
// has a single child, `route`, that prints the realized ingress view for a
// domain (issue #174).
func getCmd() *cobra.Command {
	c := &cobra.Command{
		Use:   "get",
		Short: "Inspect a single cluster resource",
	}
	c.AddCommand(getRouteCmd())
	return c
}
